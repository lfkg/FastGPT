# --------- install dependence -----------
FROM mirrors.sangfor.com/node:20.14.0-alpine AS maindeps
WORKDIR /app
...
RUN npm config set registry http://npm.uedc.sangfor.com.cn
RUN apk add --no-cache libc6-compat && npm install -g pnpm@9.4.0

# copy packages and one project
COPY ./node-xlsx-0.24.0.tgz ./
...
# --------- builder -----------
FROM mirrors.sangfor.com/node:20.14.0-alpine AS builder
WORKDIR /app
...
# --------- runner -----------
FROM mirrors.sangfor.com/node:20.14.0-alpine AS runner
WORKDIR /app

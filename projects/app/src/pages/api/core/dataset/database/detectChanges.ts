  ...
  try {
    ...
    return await mongoSessionRun(async (session) => {
      const result = await withDatabaseClient(datasetId, async (dbClient) => {
        ...
        // Step 5: 并行获取所有表的信息 (关键性能优化)
        console.log(`[detectChanges] Fetching ${databaseTableNames.length} tables in parallel...`);
        ...
        for (const result of tableInfoResults) {
          if (result.status === 'rejected') {
            console.error('[detectChanges] Failed to fetch table info:', result.reason);
            continue;
          }
        ...
        console.log('[detectChanges] Performance metrics:', {
          ...perfMarks,
          total: totalTime,
          tables: databaseTableNames.length,
          avgTimePerTable:
            databaseTableNames.length > 0
              ? Math.round(
                  (perfMarks['fetch_table_info'] - perfMarks['process_deleted']) /
                    databaseTableNames.length
                )
              : 0
        });
      ...
      console.debug('[detectChanges] result', result);
